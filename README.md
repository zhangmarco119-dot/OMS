...
StoreHub V2 当前完成阶段 0–3：V1 兼容审计、角色边界、到货数据库/RLS/私有 Storage，以及门店端草稿、图片、商品搜索、自动保存、提交、成功页和到货历史。详见 [V2 阶段 3 门店端说明](docs/V2_PHASE_3_STORE_ARRIVALS.md)。

## 当前能力

- 使用账号名或姓名登录，底层通过 Supabase Auth 校验密码并保持会话。
- 根据账号 profile 自动识别门店。
- 员工和店长首页保留“点货”“订货”，并增加 V2“到货上报”；管理员首页继续显示独立消息中心和到货中心状态。
...
V1 阶段 9 已完成。V2 当前停在阶段 3；下一步只能在确认后进入阶段 4 管理员到货中心。
