...
to authenticated
using (public.can_modify_task(id))
with check (
  public.has_store_access(store_id)
  and (
    created_by = auth.uid()
...
to authenticated
using (status = 'draft' and public.can_modify_task(id));
...
insert into public.stores (name, short_name)
values
  ('宝珠奶酪（五道口店）', '宝珠奶酪'),
  ('OMEGA酸奶（西直门店）', 'OMEGA酸奶')
...
on conflict (admin_profile_id, store_id) do nothing;