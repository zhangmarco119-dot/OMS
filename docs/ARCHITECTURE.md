...
- Supabase Auth 仍是 StoreHub 唯一登录身份；钉钉只提供员工目录、排班、考勤结果和打卡流水。
- 前端不接触第三方响应或密钥；`dingtalk-attendance` Edge Function 完成鉴权、分片、重试、标准化和幂等写入。
- 页面读取本地考勤表和安全 RPC，打开页面不会实时调用钉钉。
- 员工和店长只看本人；管理员只看员工当前所属且自己已授权的门店。考勤权限不复用店长业务管理权限。
- 自动同步使用独立 Cron Secret，每日增量回补最近 7 天，月初回补上月末 5 天。
