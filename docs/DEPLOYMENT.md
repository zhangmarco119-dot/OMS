...
3. 正式发布必须提升版本号：日常修复可提升补丁版本 `x.y.z`，重大升级可提升到 `x.(y+1).0`；同时在 `src/config/version.ts` 和 `package.json` 保持一致，并在版本历史顶部填写本次中文更新摘要。
...
- 界面版本与 `package.json` 一致；`manage-system` 的每次合并都必须提升补丁版本，或将次版本号提高一级并把补丁位归零；
