...
到货上报每次打开时都会读取设备当前本地日期和时间，并覆盖旧草稿或本地缓存中的日期/时间显示；已填写的商品、图片、快递信息和备注继续保留。
...
- 至少一个完整产品；
- 产品名称、数量和单位有效；
- 单位必须按瓶、袋、盒、个、杯、克或毫升等最小单位填写，禁止使用箱、整箱、件、整件。

到货上报页使用紧凑卡片和表单间距。提交按钮在信息不完整时保持可点击并显示灰色，点击后通过中文弹窗集中列出缺失内容，不在页面底部持续占用空间；信息完整后进入确认弹层，显示门店、产品种类、自动描述和两类图片数量。前端阻止重复点击，数据库 `submit_arrival_report` 继续使用版本和幂等键保证只生成一条正式记录和通知。
