  ...
  return (
    <section className="rounded-lg bg-white p-3 shadow-sm">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button className="flex min-h-10 items-center justify-center gap-2 rounded-lg bg-brand-600 px-3 text-sm font-bold text-white" onClick={() => cameraRef.current?.click()} type="button">
          <Camera className="h-4 w-4" aria-hidden="true" />
          直接拍照
        </button>
        <button className="flex min-h-10 items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 text-sm font-bold text-slate-800" onClick={() => albumRef.current?.click()} type="button">
      ...
      {images.length === 0 && pending.length === 0 ? (
        <div className="mt-3 rounded-lg border border-dashed border-slate-300 p-4 text-center text-sm text-slate-500">尚未上传图片</div>
      ) : (
