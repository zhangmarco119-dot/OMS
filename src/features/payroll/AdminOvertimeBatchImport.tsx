...
export function AdminOvertimeBatchImport({
  ...
  const downloadTemplate = async () => {
    try {
      ...
      setMessage(error instanceof Error ? error.message : '自主延时工作导入模板生成失败。');
    }
  };
  ...
  const startImport = async () => {
    ...
    try {
      ...
      if (!rows.length) throw new Error('Excel 中没有需要导入的自主延时工作登记。');
      ...
      setMessage(error instanceof Error ? error.message : '自主延时工作登记批量导入失败。');
    } finally {
      setImporting(false);
    }
  };
  ...
  return <>
    <SectionCard>
      <SectionHeader icon={FileSpreadsheet} title="批量导入自主延时工作登记" description="适合一次登记多名员工或多个日期；导入后直接通过并计入实时薪资。" />
    ...
    <BatchImportReportDialog failureCount={report?.failed ?? 0} failures={report?.failures ?? []} onClose={() => setReport(null)} open={Boolean(report)} successCount={report?.succeeded ?? 0} successDescription={`成功登记 ${report?.succeeded ?? 0} 条自主延时工作登记，已直接确认通过并计入实时薪资。失败行不会影响其他记录。`} title="员工自主延时工作批量导入完成" />
