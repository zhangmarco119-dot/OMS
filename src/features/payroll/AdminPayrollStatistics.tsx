...
export function AdminPayrollStatistics() {
  ...
  const [customFrom, setCustomFrom] = useRememberedPageState('payroll-statistics-from', `${currentMonth()}-01`);
  const [customTo, setCustomTo] = useRememberedPageState('payroll-statistics-to', today);
  ...
  const [search, setSearch] = useRememberedPageState('payroll-statistics-search', '');
  ...
  const download = async () => {
    ...
    try {
      ...
      setFeedback({ title: '薪资统计图表已生成', message: '图表已下载为 PNG 图片，包含总览、门店对比和员工详细工资单。', tone: 'success' });
    } catch (error) {
  ...
  const selectedEmployee = statistics?.employees.find((employee) => employee.profileId === params.get('statisticsEmployee')) ?? null;
  const selectedPeriod = selectedEmployee?.periods.find((period) => `${period.payrollMonth}|${period.from}|${period.to}` === params.get('statisticsPeriod')) ?? null;
  const openEmployee = (employee: PayrollStatisticsEmployee) => { const next = new URLSearchParams(params); next.set('statisticsEmployee', employee.profileId); next.delete('statisticsPeriod'); setParams(next); };
  const openPeriod = (period: PayrollStatisticsPeriod) => { const next = new URLSearchParams(params); next.set('statisticsPeriod', `${period.payrollMonth}|${period.from}|${period.to}`); setParams(next); };
  ...
  return <>
    ...
    {status === 'ready' && statistics ? <>
      ...
      <SectionCard className="overflow-hidden p-0"><div className="border-b border-slate-100 p-3"><SectionHeader icon={Clock3} title={`员工工资 · ${statistics.employees.length}人`} /><label className="relative mt-3 block"><Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><input className="ui-input pl-9" onChange={(event) => setSearch(event.target.value)} placeholder="搜索员工" value={search} /></label></div>{!filteredEmployees.length ? <EmptyState title="选定时间内暂无员工工资" /> : <div className="divide-y divide-slate-100">{filteredEmployees.map((employee) => <button className="ui-interactive grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-3 p-3 text-left" key={employee.profileId} onClick={() => openEmployee(employee)} type="button"><div className="min-w-0"><b className="truncate text-sm">{employee.displayName}{employee.employmentType === 'part_time' ? ' · 兼职' : ''}</b><p className="mt-1 text-xs text-slate-500">{employee.hours.toFixed(2)} 小时 · 平均 {employee.averageHourlyCost == null ? '—' : `${formatMoney(employee.averageHourlyCost)}/小时`}</p></div><div className="flex items-center gap-1.5"><b className="tabular-nums text-brand-800">{formatMoney(employee.salaryCost)}</b><ChevronRight className="h-4 w-4 text-slate-400" /></div></button>)}</div>}</SectionCard>
    </> : null}
