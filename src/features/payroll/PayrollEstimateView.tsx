...
export function PayrollEstimateView({ estimate }: { estimate: PayrollEstimate }) {
  ...
  return <>
    ...
    <SectionCard><SectionHeader icon={CalendarDays} title="出勤与计薪口径" description="同一天跨门店出勤只计 1 天，不重复计薪。" /><div className="mt-3 grid grid-cols-3 gap-2 text-center"><Metric label="累计出勤" value={`${estimate.attendanceDays} 天`} /><Metric label="全勤标准" value={`${estimate.fullAttendanceDays} 天`} /><Metric label="迟到" value={`${estimate.lateCount} 次`} /></div></SectionCard>
