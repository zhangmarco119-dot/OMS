  ...
  return <>
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      ...
      {images.map((image, index) => {
        ...
        const canDelete = onDelete && !uploading && (!deletableImageIds || deletableImageIds.includes(image.id));
        return <div className="overflow-hidden rounded-lg border bg-slate-50" key={image.id}>
          <button aria-label={`全屏查看${alt}`} className="relative block w-full text-left" onClick={() => setActiveImage({ alt, url })} type="button">
            <img alt={alt} className="aspect-square w-full object-cover" src={url} />
            {uploading ? <div aria-label={`${alt}上传进度`} aria-valuemax={100} aria-valuemin={0} aria-valuenow={uploadProgress} className="absolute inset-x-2 bottom-2 rounded-md bg-slate-950/75 px-2 py-1.5 text-white" role="progressbar">
          ...
          <div className="flex min-h-10 items-center justify-between gap-2 px-2 py-1">
            <span className="truncate text-xs text-slate-600">{uploading ? '上传完成后即可提交' : '点击放大查看'}</span>
            {canDelete ? <button aria-label={`删除${alt}`} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-red-50 text-red-700 disabled:opacity-50" disabled={deleting} onClick={() => onDelete(image)} type="button">{deleting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}</button> : null}
          </div>
        </div>;
      })}
    </div>
