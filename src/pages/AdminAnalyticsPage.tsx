...
export function AdminAnalyticsPage() {
  ...
  return <PageShell eyebrow="门店运营系统 · 管理员" title="运营数据统计" backTo="/app">
    ...
    {status === 'ready' ? <>
      ...
      <section className="rounded-lg bg-white p-4 shadow-sm"><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-sky-700" /><h2 className="font-bold text-slate-900">V1 摘要</h2></div><div className="mt-3 grid grid-cols-3 gap-2 text-center"><Metric label="盘点提交" to="/app/history" value={data.v1.inventory_submissions} /><Metric label="订货提交" to="/app/history" value={data.v1.order_submissions} /><Metric label="未完成盘点" to="/app/inventory" value={data.v1.open_inventory} /></div></section>
    </> : null}
  </PageShell>;
}

function Metric({ label, to, value }: { label: string; to: string; value: number | string }) {
  return <Link className="rounded-lg bg-slate-50 px-2 py-3 transition hover:bg-brand-50 active:scale-[0.99]" to={to}><p className="text-lg font-bold text-slate-900">{value}</p><p className="mt-1 text-[11px] text-slate-500">{label}</p></Link>;
}
