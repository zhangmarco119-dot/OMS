...
export function AdminArrivalSummaryPage() {
  ...
  const load = useCallback(async () => {
    ...
    setStatus('loading');
    try { setSummary(await loadAdminArrivalSummary(supabase, date, storeId)); setStatus('ready'); setMessage(null); }
...
function Empty() { return <p className="rounded-lg bg-white p-8 text-center text-slate-500 shadow-sm">当天没有符合条件的到货数据。</p>; }
