  ...
  return <>
    <SectionCard className="p-3.5">
      ...
      <details className="mt-3 rounded-lg border border-slate-200 p-3">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2"><label className="text-xs font-semibold text-slate-600">开始月份<input className="ui-input mt-1" max={`${historyEnd}-01`} onChange={(event) => setHistoryStart(event.target.value.slice(0, 7))} type="date" value={`${historyStart}-01`} /></label><label className="text-xs font-semibold text-slate-600">结束月份<input className="ui-input mt-1" max={`${currentMonth()}-01`} min={`${historyStart}-01`} onChange={(event) => setHistoryEnd(event.target.value.slice(0, 7))} type="date" value={`${historyEnd}-01`} /></label></div>
