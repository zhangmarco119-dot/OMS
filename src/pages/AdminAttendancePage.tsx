...
function MiniMetric({ compact = false, label, value }: { compact?: boolean; label: string; value: number | string }) { return <div className="min-w-0 rounded-lg bg-slate-50 px-1 py-2"><b className={`block whitespace-nowrap tabular-nums text-slate-900 ${compact ? 'text-[11px]' : ''}`}>{value}</b><span className="text-[10px] text-slate-500">{label}</span></div>; }
