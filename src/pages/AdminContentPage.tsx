...
function NoticeEditor({ busy, draft, onCancel, onChange, onPublish, onSave, onUpload, recipients, stores }: { busy: boolean; draft: NoticeDraft; onCancel: () => void; onChange: (value: NoticeDraft) => void; onPublish: () => void; onSave: () => void; onUpload: (file: File | undefined) => Promise<void>; recipients: ContentRecipient[]; stores: Array<{ id: string; name: string }> }) {
  ...
  return <div className="fixed inset-0 z-40 overflow-y-auto bg-[#f4f7f3] p-4" role="dialog" aria-modal="true"><div className="mx-auto max-w-3xl space-y-4 pb-24"><header className="sticky top-0 z-10 flex items-center justify-between rounded-lg bg-white p-4 shadow-sm"><div><p className="text-xs font-bold text-brand-700">公告编辑</p><h2 className="text-xl font-bold">{draft.id ? '编辑公告' : '新建公告'}</h2></div><button aria-label="关闭公告编辑" className="h-11 w-11 rounded-lg bg-slate-100" onClick={onCancel} type="button"><X className="mx-auto h-5 w-5" /></button></header><section className="space-y-3 rounded-lg bg-white p-4 shadow-sm"><label className="block text-sm font-semibold">公告标题<input className="mt-1 min-h-11 w-full rounded-lg border p-3" onChange={(event) => onChange({ ...draft, title: event.target.value })} value={draft.title} /></label><label className="block text-sm font-semibold">公告正文<textarea className="mt-1 min-h-40 w-full rounded-lg border p-3" onChange={(event) => onChange({ ...draft, body: event.target.value })} value={draft.body} /></label><label className="block text-sm font-semibold">失效时间（可选）<input className="mt-1 min-h-11 w-full rounded-lg border p-3" onChange={(event) => onChange({ ...draft, expiresAt: event.target.value })} type="datetime-local" value={draft.expiresAt} /></label><label className="flex min-h-11 items-center gap-2 text-sm font-semibold"><input checked={draft.isPinned} onChange={(event) => onChange({ ...draft, isPinned: event.target.checked })} type="checkbox" />置顶显示</label><label className="flex min-h-11 items-center gap-2 text-sm font-semibold"><input checked={draft.requiresAcknowledgment} onChange={(event) => onChange({ ...draft, requiresAcknowledgment: event.target.checked })} type="checkbox" />要求接收人确认（计入待办）</label><StorePicker onChange={(storeIds) => onChange({ ...draft, recipientIds: draft.recipientIds.filter((id) => recipients.some((recipient) => recipient.id === id && storeIds.includes(recipient.store_id))), storeIds })} selected={draft.storeIds} stores={stores} /><div><div className="flex items-center justify-between"><p className="text-sm font-semibold">接收人员（已选 {draft.recipientIds.length} 人）</p><button className="min-h-10 rounded-lg border px-3 text-sm font-bold text-brand-700" onClick={selectAll} type="button">全选当前结果</button></div><div className="mt-2 flex gap-2"><button className={`min-h-10 rounded-lg border px-3 text-sm ${roleFilter === 'all' ? 'border-brand-600 bg-brand-50 text-brand-700' : ''}`} onClick={() => setRoleFilter('all')} type="button">全部角色</button><button className={`min-h-10 rounded-lg border px-3 text-sm ${roleFilter === 'staff' ? 'border-brand-600 bg-brand-50 text-brand-700' : ''}`} onClick={() => setRoleFilter('staff')} type="button">员工</button><button className={`min-h-10 rounded-lg border px-3 text-sm ${roleFilter === 'manager' ? 'border-brand-600 bg-brand-50 text-brand-700' : ''}`} onClick={() => setRoleFilter('manager')} type="button">店长</button></div><div className="mt-2 grid gap-2 sm:grid-cols-2">{visibleRecipients.map((recipient) => <label className="flex min-h-11 items-center gap-2 rounded-lg border px-3 text-sm" key={recipient.id}><input checked={draft.recipientIds.includes(recipient.id)} onChange={() => onChange({ ...draft, recipientIds: draft.recipientIds.includes(recipient.id) ? draft.recipientIds.filter((id) => id !== recipient.id) : [...draft.recipientIds, recipient.id] })} type="checkbox" />{recipient.display_name} · {recipient.role === 'staff' ? '员工' : '店长'}</label>)}</div></div>{draft.id ? <label className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-slate-200 px-3 text-sm font-bold text-brand-700"><FileUp className="h-4 w-4" />上传图片或 PDF<input accept="image/jpeg,image/png,image/webp,application/pdf" className="hidden" disabled={busy} onChange={(event) => { void onUpload(event.target.files?.[0]); event.currentTarget.value = ''; }} type="file" /></label> : <p className="text-sm text-slate-500">保存草稿后即可上传图片或 PDF 附件。</p>}</section><EditorActions busy={busy} onCancel={onCancel} onPublish={onPublish} onSave={onSave} /></div></div>;
}
