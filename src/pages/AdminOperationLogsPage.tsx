...
export function AdminOperationLogsPage() {
  ...
  const more = async () => {
    ...
    setLoadingMore(true);
    try {
      const result = await loadOperationLogs(supabase, filters, items.length);
  ...
  return <PageShell backTo="/app/workbench" contentGapClassName="gap-3" eyebrow="门店运营系统 · 管理员" title="操作日志">
    ...
    {status === 'loading' ? <LoadingState label="正在加载操作日志" /> : status === 'error' ? <ErrorState message="暂时无法读取操作日志。" onRetry={() => void load()} /> : !items.length ? <EmptyState title="暂无匹配的操作日志" description="可以调整账号、模块、操作或日期筛选条件。" /> : <>
      ...
      <div className="space-y-2">{items.map((row) => {
        ...
        const detailEntries = Object.entries(row.metadata).filter(([key, value]) => key !== 'eventKey' && key !== 'targetProfileId' && value != null && value !== '' && !(Array.isArray(value) && value.length === 0));
        return <SectionCard className="p-3.5" key={row.id}>
          ...
          <p className="mt-2 text-sm font-semibold text-slate-700">{moduleLabels[row.module] ?? row.module} · {row.summary}</p>
          <p className="mt-1 text-xs text-slate-500">{formatTime(row.occurred_at)}{storeName ? ` · ${storeName}` : ''}</p>
          <button className="mt-2 flex min-h-8 items-center gap-1 text-xs font-bold text-brand-700" onClick={() => setExpandedId(expanded ? null : row.id)} type="button">{expanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}{expanded ? '收起详情' : '查看详情'}</button>
      ...
      {items.length < total ? <button className="ui-button-secondary w-full" disabled={loadingMore} onClick={() => void more()} type="button">{loadingMore ? '正在加载' : '加载更多'}</button> : null}
    </>}
  </PageShell>;
}
