...
import { AdminPayrollStatistics } from '../features/payroll/AdminPayrollStatistics';
...
type Tab = 'overview' | 'statistics' | 'payslips' | 'employees' | 'performance' | 'revenue' | 'penalties' | 'overtime';
...
const tabs: { key: Tab; label: string }[] = [
  ...
  { key: 'penalties', label: '处罚' }, { key: 'overtime', label: '工时计薪' },
];
...
function PayrollPayslipManager() {
  ...
  const [sendTarget, setSendTarget] = useState<'employee' | 'manager'>('employee');
  const [confirmationManagers, setConfirmationManagers] = useState<Awaited<ReturnType<typeof listPayrollConfirmationManagers>>>([]);
  const [confirmationManagerId, setConfirmationManagerId] = useState('');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const requested = routeParams.get('payslip');
      setViewing((current) => requested ? nextItems.find((item) => item.id === requested) ?? null : current ? nextItems.find((item) => item.id === current.id) ?? null : null);
    } catch { setStatus('error'); }
  }, [month, routeParams, setProfileId]);
  useEffect(() => { void load(); }, [load]);
  useEffect(() => {
    if (!routeParams.has('payslip')) setViewing(null);
    if (!routeParams.has('editPayslip')) setEditing(null);
  }, [routeParams]);
  useEffect(() => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (result.missingTaxCount) parts.push(`${result.missingTaxCount} 人个税尚未登记`);
      setFeedback({ title: '工资单草稿已生成', message: `${parts.join('，')}。${result.missingTaxCount ? '未登记者暂按 0 元个税生成，请先到“税务与记账 · 个税登记”补齐；' : ''}请逐份预览，确认无误后再发送。`, tone: result.missingTaxCount ? 'warning' : 'success' });
      await load();
  ...
  const beginEdit = (item: Payslip) => {
    ...
    const next = new URLSearchParams(routeParams); next.delete('payslip'); next.set('editPayslip', item.id); setRouteParams(next);
    setEditForm({
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const wasConfirmed = editing.status === 'confirmed'; setEditing(null); const next = new URLSearchParams(routeParams); next.delete('editPayslip'); setRouteParams(next, { replace: true });
  ...
  const beginSend = async (item: Payslip) => {
    setAction({ item, type: 'send' });
    setSendTarget('employee');
    setConfirmationManagers([]);
    setConfirmationManagerId('');
    if (!supabase) return;
    try {
      const managers = await listPayrollConfirmationManagers(supabase, item.id);
      setConfirmationManagers(managers);
      setConfirmationManagerId(managers[0]?.id ?? '');
    } catch (error) {
      setFeedback({ title: '店长名单加载失败', message: error instanceof Error ? error.message : '请稍后重试。', tone: 'warning' });
    }
  };
  const executeAction = async () => {
    ...
    try {
      if (current.type === 'send' && sendTarget === 'manager') {
        if (!confirmationManagerId) { setFeedback({ title: '请选择店长', message: '请选择负责线下核对并确认该工资单的店长。', tone: 'warning' }); setBusy(false); return; }
        await sendPayrollPayslipToManager(supabase, current.item.id, confirmationManagerId);
      } else if (current.type === 'send') await sendPayrollPayslip(supabase,current.item.id); else await withdrawPayrollPayslip(supabase,current.item.id);
      setViewing(null); const next = new URLSearchParams(routeParams); next.delete('payslip'); next.delete('editPayslip'); setRouteParams(next, { replace: true });
      setFeedback({ title: current.type === 'send' ? '工资单已发送' : '工资单已撤回', message: current.type === 'send' ? sendTarget === 'manager' ? '工资单已发送给所选店长，店长待办已经生成。' : '员工通知和确认待办已经生成。' : '工资单、通知和确认待办已同步移除。', tone: 'success' });
      await load();
  ...
  const statusLabel = (item: Payslip) => item.status === 'draft' ? '待发送' : item.status === 'issued' ? item.confirmation_target === 'manager' ? '待店长确认' : '待员工确认' : item.status === 'confirmed' ? item.confirmation_target === 'manager' ? '店长已确认' : '员工已确认' : '已撤回';
  const statusTone = (item: Payslip) => item.status === 'confirmed' ? 'success' : item.status === 'withdrawn' ? 'danger' : item.status === 'draft' ? 'info' : 'warning';
  const actionDialogContent = action?.type === 'send' ? <div className="space-y-3"><p className="text-sm leading-6 text-slate-600">请选择由谁确认这份工资单。发送给店长适用于兼职等不使用系统的员工。</p><label className="flex items-start gap-2 rounded-lg border border-slate-200 p-3 text-sm"><input checked={sendTarget === 'employee'} className="mt-1" onChange={() => setSendTarget('employee')} type="radio" /><span><b className="block">员工本人确认</b><span className="mt-1 block text-xs text-slate-500">员工收到工资单通知和确认待办。</span></span></label><label className="flex items-start gap-2 rounded-lg border border-slate-200 p-3 text-sm"><input checked={sendTarget === 'manager'} className="mt-1" disabled={!confirmationManagers.length} onChange={() => setSendTarget('manager')} type="radio" /><span><b className="block">发送给店长确认</b><span className="mt-1 block text-xs text-slate-500">店长先在线下找员工核对，再在系统中代为确认。</span></span></label>{sendTarget === 'manager' ? <label className="block text-sm font-semibold">选择店长<select className="ui-input mt-1" onChange={(event) => setConfirmationManagerId(event.target.value)} value={confirmationManagerId}><option value="">请选择店长</option>{confirmationManagers.map((manager) => <option key={manager.id} value={manager.id}>{manager.displayName}{manager.storeName ? ` · ${manager.storeName}` : ''}</option>)}</select></label> : null}{!confirmationManagers.length ? <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800">该工资单所属门店目前没有可选的在职店长，只能发送给员工本人确认。</p> : null}</div> : <p>撤回后，该工资单及对应通知、确认待办会同步失效。</p>;
  if (viewing) return <><button className="ui-button-secondary" onClick={closePayslipChild} type="button">返回工资单列表</button><div className="flex items-center justify-between gap-2"><StatusBadge tone={statusTone(viewing)}>{statusLabel(viewing)}</StatusBadge><span className="text-xs text-slate-500">第 {viewing.revision} 版</span></div><PayrollStatementView adminNote={viewing.admin_note} estimate={viewing.estimate} payrollMonth={viewing.payroll_month} /><div className="grid grid-cols-2 gap-2"><button className="ui-button-secondary" disabled={viewing.status === 'withdrawn'} onClick={() => beginEdit(viewing)} type="button">修改</button>{viewing.status === 'draft' ? <button className="ui-button-primary" onClick={() => void beginSend(viewing)} type="button">发送工资单</button> : <button className="ui-button-danger" disabled={viewing.status === 'withdrawn'} onClick={() => setAction({ item:viewing,type:'withdraw' })} type="button">撤回工资单</button>}</div><ConfirmDialog confirmLabel={action?.type === 'send' ? '确认发送' : '确认撤回'} danger={action?.type === 'withdraw'} onCancel={() => setAction(null)} onConfirm={() => void executeAction()} open={Boolean(action)} title={action?.type === 'send' ? '发送工资单' : '撤回工资单'}>{actionDialogContent}</ConfirmDialog><ActionFeedbackDialog message={feedback?.message ?? ''} onClose={() => setFeedback(null)} open={Boolean(feedback)} title={feedback?.title ?? ''} tone={feedback?.tone} /></>;
  if (editing) return <><button className="ui-button-secondary" onClick={closePayslipChild} type="button">取消修改</button><SectionCard><SectionHeader title="修改工资单" description={editing.status === 'confirmed' ? '此工资单已被员工确认。保存修改后，系统会要求员工重新确认。' : '修改金额后，实发合计会自动重新计算。'} /><div className="mt-3 grid grid-cols-2 gap-2">{([['accruedBaseSalary','基本工资'],['accruedHousingAllowance','房补'],['accruedPerformance','绩效'],['accruedFullAttendanceBonus','全勤奖'],['accruedExtraAttendanceBonus','超勤奖'],['accruedServiceAward','工龄奖'],['accruedExtraReward','额外奖励'],['accruedCommission','提成'],['accruedOvertime','延时工作补贴'],['fineTotal','罚款'],['individualIncomeTax','个税扣除']] as const).map(([key,label]) => <label className="text-sm font-semibold" key={key}>{label}<input className="ui-input mt-1" min="0" onChange={(event) => setEditForm((value) => ({ ...value,[key]:event.target.value }))} step="0.01" type="number" value={editForm[key] ?? ''} /></label>)}</div><label className="mt-3 block text-sm font-semibold">工资单备注（选填）<textarea className="ui-input mt-1 min-h-20 py-2" onChange={(event) => setEditForm((value) => ({ ...value,adminNote:event.target.value }))} value={editForm.adminNote ?? ''} /></label><button className="ui-button-primary mt-3 w-full" disabled={busy} onClick={() => void saveEdit()} type="button">保存工资单修改</button></SectionCard><ActionFeedbackDialog message={feedback?.message ?? ''} onClose={() => setFeedback(null)} open={Boolean(feedback)} title={feedback?.title ?? ''} tone={feedback?.tone} /></>;
  return <>
    ...
    <SectionCard><SectionHeader icon={FileText} title="生成工资单" description="草稿会读取“个税登记”中的本月实际个税，生成后请预览再发送。" />
    ...
    <SectionCard><SectionHeader title={`${month.slice(0,4)}年${Number(month.slice(5,7))}月工资单`} description={`共 ${items.length} 份 · 待发送 ${items.filter((item) => item.status==='draft').length} · 待确认 ${items.filter((item) => item.status==='issued').length} · 已确认 ${items.filter((item) => item.status==='confirmed').length}`} />
      ...
      {status === 'ready' ? <div className="mt-3 space-y-2">{items.map((item) => { const profile=profiles.find((entry)=>entry.id===item.profile_id); return <article className="rounded-lg border border-slate-200 bg-white p-3" key={item.id}><button className="w-full text-left" onClick={() => openPayslip(item)} type="button"><div className="flex items-start justify-between gap-3"><div><b>{profile?.display_name??item.estimate.displayName}</b><p className="mt-1 text-xs text-slate-500">实发 {formatMoney(item.estimate.estimatedPayable??item.estimate.knownEstimatedPayable)} · 第 {item.revision} 版</p></div><StatusBadge tone={statusTone(item)}>{statusLabel(item)}</StatusBadge></div></button><div className="mt-3 grid grid-cols-3 gap-2"><button className="ui-button-secondary min-h-9 px-2 text-xs" onClick={() => openPayslip(item)} type="button">查看</button><button className="ui-button-secondary min-h-9 px-2 text-xs" disabled={item.status==='withdrawn'} onClick={() => beginEdit(item)} type="button">修改</button>{item.status==='draft' ? <button className="ui-button-primary min-h-9 px-2 text-xs" onClick={() => void beginSend(item)} type="button">发送</button> : <button className="ui-button-danger min-h-9 px-2 text-xs" disabled={item.status==='withdrawn'} onClick={() => setAction({ item,type:'withdraw' })} type="button">撤回</button>}</div></article>; })}</div> : null}
    </SectionCard>
    <ConfirmDialog confirmLabel="确认生成" onCancel={() => setGenerateOpen(false)} onConfirm={() => void generate()} open={generateOpen} title="生成工资单草稿"><p>系统会读取所选月份的实际个税，只生成草稿，不会立即通知员工。请生成后逐份查看，确认无误再发送。</p></ConfirmDialog>
    <ConfirmDialog confirmLabel={action?.type==='send'?'确认发送':'确认撤回'} danger={action?.type==='withdraw'} onCancel={() => setAction(null)} onConfirm={() => void executeAction()} open={Boolean(action)} title={action?.type==='send'?'发送工资单':'撤回工资单'}>{actionDialogContent}</ConfirmDialog>
...
function EmployeeRules() {
  ...
  if (selectedProfile?.employment_type === 'part_time') return <><PayrollVisibilityManager /><SectionCard><SectionHeader icon={Settings2} title="兼职员工工资参数" description="兼职账号只按审批通过的兼职工时计薪。" /><label className="mt-3 block text-sm font-semibold">选择员工<select className="ui-input mt-1" onChange={(event) => setProfileId(event.target.value)} value={profileId}>{setup.profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.display_name} · {payrollProfileLabel(profile)}</option>)}</select></label><p className="mt-3 rounded-lg bg-emerald-50 p-3 text-sm leading-6 text-emerald-900">兼职薪资 = 审批通过的兼职工时 × 审批时锁定的兼职时薪。请在“工时与计薪”菜单设置时薪，店长审批通过后自动计入。</p></SectionCard><FeedbackDialog feedback={feedback} close={() => setFeedback(null)} /></>;
  return <><PayrollVisibilityManager /><SectionCard>
...
function AttendanceAllocationRuleCard({ busy, form, onChange, onSave, stores }: {
  ...
  return <SectionCard>
    ...
    <p className="mt-2 rounded-lg bg-blue-50 px-3 py-2 text-xs leading-5 text-blue-900">例如设置 50%：当天满足外勤条件后，排班门店承担 50%，分摊门店承担 50%；自主延时工作仍按审批记录中的实际门店单独计算。</p>
...
function PenaltyManager() {
  ...
  const [detailId, setDetailId] = useState<string | null>(null); const [detailImageIndex, setDetailImageIndex] = useState<number | null>(null); const [detailImageUrls, setDetailImageUrls] = useState<Record<string, string | null>>({});
  ...
  const detail = setup?.penalties.find((row) => row.id === detailId); const detailAssets = setup?.penaltyAssets.filter((asset) => asset.penalty_id === detailId) ?? [];
  useEffect(() => {
    if (!detailId || !supabase) return;
    const client = supabase; const assets = setup?.penaltyAssets.filter((asset) => asset.penalty_id === detailId) ?? [];
    let cancelled = false; setDetailImageUrls({});
    for (const asset of assets) void loadPayrollPenaltyAssetUrl(client, asset).then((url) => { if (!cancelled) setDetailImageUrls((current) => ({ ...current, [asset.id]: url })); }).catch(() => { if (!cancelled) setDetailImageUrls((current) => ({ ...current, [asset.id]: null })); });
    return () => { cancelled = true; };
  }, [detailId, setup?.penaltyAssets]);
  const detailImages = detailAssets.filter((asset) => Boolean(detailImageUrls[asset.id])).map((asset) => ({ alt: asset.file_name, url: detailImageUrls[asset.id] as string }));
  ...
  const publisherLabel = (createdBy: string) => {
    const publisher = setup?.penaltyPublishers.find((profile) => profile.id === createdBy);
    const name = publisher?.display_name ?? (createdBy === auth.profile?.id ? auth.profile.display_name : null) ?? '未知发布人';
    const role = publisher?.role === 'manager' ? '店长' : publisher?.role === 'admin' ? '管理员' : '';
    return `${name}${role ? ` · ${role}` : ''}`;
  };
  return <><SectionCard><SectionHeader icon={ClipboardPen} title="其他罚款与纪律记录" description="事件等级会自动带出默认绩效扣分，仍可按实际情况调整。" /><label className="mt-3 block text-sm font-semibold">员工<select className="ui-input mt-1" onChange={(event) => setProfileId(event.target.value)} value={profileId}>{setup?.profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.display_name}</option>)}</select></label><div className="mt-3 grid grid-cols-2 gap-2"><label className="text-sm font-semibold">日期<input className="ui-input mt-1" max={todayInChina()} onChange={(event) => setDate(event.target.value)} type="date" value={date} /></label><label className="text-sm font-semibold">事件等级<select className="ui-input mt-1" onChange={(event) => changeLevel(event.target.value as keyof typeof penaltyDefaults)} value={level}><option value="reminder">提醒（默认 0 分）</option><option value="warning">警告（默认 3 分）</option><option value="formal_warning">正式警告（默认 5 分）</option><option value="serious">严重违纪（默认 10 分）</option></select></label><Field label="罚款金额" value={amount} onChange={setAmount} /><Field label="绩效扣分" value={deduction} onChange={setDeduction} /></div><label className="mt-3 block text-sm font-semibold">原因<input className="ui-input mt-1" onChange={(event) => setReason(event.target.value)} value={reason} /></label><label className="ui-button-secondary mt-3 w-full cursor-pointer"><Camera className="h-4 w-4" />上传处罚图片（选填）<input accept="image/jpeg,image/png,image/webp" className="sr-only" multiple onChange={(event) => setFiles(Array.from(event.target.files ?? []))} type="file" /></label>{previews.length ? <div className="mt-2 grid grid-cols-3 gap-2">{previews.map((item) => <div className="relative" key={`${item.file.name}-${item.file.lastModified}`}><button className="block w-full" onClick={() => setActiveImageIndex(previews.findIndex((entry) => entry.url === item.url))} type="button"><img alt={item.file.name} className="aspect-square w-full rounded-lg object-cover" src={item.url} /></button><button aria-label={`删除 ${item.file.name}`} className="absolute right-1 top-1 flex h-7 w-7 min-h-0 items-center justify-center rounded-full bg-black/70 text-white" onClick={() => setFiles((current) => current.filter((file) => file !== item.file))} type="button"><X className="h-4 w-4" /></button></div>)}</div> : null}<button className="ui-button-primary mt-3 w-full" disabled={busy} onClick={() => void save()} type="button">{busy ? '正在发布' : '发布处罚记录'}</button></SectionCard>
<section className="space-y-2">{setup?.penalties.map((row) => <SectionCard className="overflow-hidden p-0" key={row.id}><button aria-label={`查看 ${setup.profiles.find((profile) => profile.id === row.profile_id)?.display_name ?? '员工'} 的处罚详情`} className="block w-full p-3 text-left" onClick={() => setDetailId(row.id)} type="button"><div className="flex items-start justify-between gap-3"><span><b>{setup.profiles.find((profile) => profile.id === row.profile_id)?.display_name}</b><small className="block text-slate-500">{row.event_date} · {row.reason} · 扣 {row.performance_deduction} 分</small><small className="mt-2 block text-slate-500">发布人：{publisherLabel(row.created_by)}</small></span><StatusBadge tone={row.status === 'active' ? 'danger' : 'info'}>{row.status === 'active' ? `-${formatMoney(row.amount)}` : '已撤销'}</StatusBadge></div></button></SectionCard>)}</section>
<ConfirmDialog confirmLabel="确认撤销" onCancel={() => setRevokeId('')} onConfirm={() => void revoke()} open={Boolean(revokeId)} title="确认撤销处罚记录"><label className="block text-sm font-semibold">撤销原因<input className="ui-input mt-1" onChange={(event) => setRevokeReason(event.target.value)} value={revokeReason} /></label></ConfirmDialog>
{detail ? <div aria-modal="true" className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/45 p-3 sm:items-center" role="dialog" aria-label="处罚记录详情"><section className="max-h-[82vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl"><header className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-slate-900">处罚记录详情</h3><p className="mt-1 text-xs text-slate-500">{setup?.profiles.find((profile) => profile.id === detail.profile_id)?.display_name ?? '员工'} · {detail.event_date}</p></div><button aria-label="关闭处罚记录详情" className="ui-icon-button" onClick={() => setDetailId(null)} type="button"><X className="h-5 w-5" /></button></header><div className="mt-4 grid grid-cols-2 gap-2 text-sm"><div className="rounded-lg bg-slate-50 p-3"><span className="block text-xs text-slate-500">事件等级</span><b className="mt-1 block">{{ reminder: '提醒', warning: '警告', formal_warning: '正式警告', serious: '严重违纪' }[detail.event_level]}</b></div><div className="rounded-lg bg-slate-50 p-3"><span className="block text-xs text-slate-500">状态</span><b className="mt-1 block">{detail.status === 'active' ? '生效中' : '已撤销'}</b></div><div className="rounded-lg bg-slate-50 p-3"><span className="block text-xs text-slate-500">罚款金额</span><b className="mt-1 block text-rose-700">{formatMoney(detail.amount)}</b></div><div className="rounded-lg bg-slate-50 p-3"><span className="block text-xs text-slate-500">绩效扣分</span><b className="mt-1 block">{detail.performance_deduction} 分</b></div></div><div className="mt-3 rounded-lg bg-slate-50 p-3"><span className="text-xs text-slate-500">处罚原因</span><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-800">{detail.reason}</p></div>{detail.revoke_reason ? <div className="mt-3 rounded-lg bg-amber-50 p-3"><span className="text-xs text-amber-700">撤销原因</span><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-amber-900">{detail.revoke_reason}</p></div> : null}<div className="mt-4"><b className="text-sm text-slate-900">处罚图片</b>{detailAssets.length ? <div className="mt-2 grid grid-cols-3 gap-2">{detailAssets.map((asset) => { const url = detailImageUrls[asset.id]; return <button aria-label={`查看处罚图片 ${asset.file_name}`} className="block" disabled={!url} key={asset.id} onClick={() => { const index = detailImages.findIndex((image) => image.url === url); if (index >= 0) setDetailImageIndex(index); }} type="button"><ProgressiveImage alt={asset.file_name} className="aspect-square h-full w-full object-cover" containerClassName="aspect-square w-full rounded-lg" resourceLoading={url === undefined} src={url} /></button>; })}</div> : <p className="mt-2 rounded-lg bg-slate-50 p-3 text-sm text-slate-500">未上传处罚图片。</p>}</div>
{detail.status === 'active' ? <button className="ui-button-secondary mt-4 w-full text-rose-700" onClick={() => { setDetailId(null); setRevokeId(detail.id); }} type="button">撤销此记录</button> : null}<button className="ui-button-primary mt-2 w-full" onClick={() => setDetailId(null)} type="button">关闭</button></section></div> : null}{activeImageIndex !== null ? <ImageViewer activeIndex={activeImageIndex} images={previews.map((item) => ({ alt: item.file.name, url: item.url }))} label="处罚图片预览" onClose={() => setActiveImageIndex(null)} onIndexChange={setActiveImageIndex} /> : null}{detailImageIndex !== null ? <ImageViewer activeIndex={detailImageIndex} images={detailImages} label="处罚详情图片预览" onClose={() => setDetailImageIndex(null)} onIndexChange={setDetailImageIndex} /> : null}<FeedbackDialog feedback={feedback} close={() => setFeedback(null)} /></>;
}
...
function OvertimeManager() {
  ...
  const reviewTerm = reviewProfile?.employment_type === 'part_time' ? '兼职工时' : '自主延时工作登记';
...
function AdminOvertimeEntry({ onSaved, setup }: { onSaved: () => Promise<void>; setup: Setup | null }) {
  ...
  const hoursTerm = isPartTime ? '兼职工时' : '自主延时工作登记';
  ...
  const save = async () => {
    ...
    try {
      ...
      await onSaved();
      setReason('');
      setFeedback({ title: '工时已登记', message: `该记录已直接通过并计入${isPartTime ? '兼职薪资' : '员工实时薪资'}，员工通知也已发送。`, tone: 'success' });
    } catch (error) {
  ...
  return <SectionCard>
    <SectionHeader icon={Clock3} title="手动登记工时" description="管理员登记后直接生效，无需员工再次申请或审批。" />
    <div className="mt-3 grid grid-cols-2 gap-2">
      ...
      <label className="text-sm font-semibold">{isPartTime ? '兼职日期' : '延时工作日期'}<input className="ui-input mt-1" max={todayInChina()} onChange={(event) => setDate(event.target.value)} type="date" value={date} /></label>
    ...
    <button className="ui-button-primary mt-3 w-full" disabled={busy || !profiles.length} onClick={requestSave} type="button">{busy ? '正在登记' : isPartTime ? '登记兼职工时' : '自主延时工作登记'}</button>
    {!profiles.length ? <p className="mt-2 text-xs text-amber-700">当前没有可登记工时的员工、店长或兼职员工。</p> : null}
    <ConfirmDialog confirmLabel="确认登记" onCancel={() => setConfirming(false)} onConfirm={() => void save()} open={confirming} title={isPartTime ? '确认登记兼职工时' : '确认自主延时工作登记'}><p className="text-sm text-slate-600">{selectedProfile?.display_name ?? '员工'} · {date} · {hours} 小时。确认后将立即计入薪资统计。</p></ConfirmDialog>
    <FeedbackDialog feedback={feedback} close={() => setFeedback(null)} />
  </SectionCard>;
}
