...
import { createV2TaskSchedule, loadV2TaskRecipients, loadV2TaskSchedules, loadV2Tasks, pauseV2TaskSchedule, publishV2Tasks, resumeV2TaskSchedule, type V2TaskRecipient, type V2TaskRow, type V2TaskScheduleRow } from '../services/v2-tasks.service';
...
export function AdminV2TasksPage({ publisherOnly = false }: { publisherOnly?: boolean }) {
  ...
  const load = useCallback(async () => {
    if (!supabase) return;
    try {
      const [nextTemplates, instances, nextSchedules, nextRecipients] = await Promise.all([loadTaskTemplates(supabase), loadV2Tasks(supabase), loadV2TaskSchedules(supabase), loadV2TaskRecipients(supabase)]);
  ...
  const validate = () => {
    ...
    if (recipientMode === 'employee' && !selectedProfileId) return '请选择要单独接收任务的员工或店长。';
  ...
  const publish = async () => {
    ...
    try {
      if (creationMode === 'single') {
        await publishV2Tasks(supabase, templateId, storeIds, new Date(due).toISOString(), recipientMode === 'employee' ? [selectedProfileId] : []);
        setMessage(recipientMode === 'employee' ? '单次任务已单独发布给所选员工。' : '单次任务已发布，所选门店的员工和店长现在可以查看。');
      } else {
        await createV2TaskSchedule(supabase, { firstDueAt: new Date(due).toISOString(), intervalDays: scheduleType === 'interval_days' ? intervalDays : null, monthDay: scheduleType === 'monthly' ? monthDay : null, profileIds: recipientMode === 'employee' ? [selectedProfileId] : [], scheduleType, storeIds, templateId, weekdays: scheduleType === 'weekly' ? weekdays : [] });
        setMessage(recipientMode === 'employee' ? '个人周期任务已创建，今后只会自动推送给所选员工。' : '周期任务计划已创建，首个任务已推送；之后系统会自动按周期推送。');
      }
      await load();
  ...
  return <PageShell eyebrow="门店运营系统 · 管理员" title={publisherOnly ? '任务发布' : '任务管理'} backTo="/app/workbench">
    ...
    {creatorOpen ? <section className="rounded-lg bg-white p-4 shadow-sm">
      ...
      <fieldset className="mt-4"><legend className="text-sm font-semibold">发布对象</legend><div className="mt-2 grid grid-cols-2 gap-3"><label className={`rounded-lg border p-3 text-sm ${recipientMode === 'stores' ? 'border-brand-600 bg-brand-50' : ''}`}><input checked={recipientMode === 'stores'} onChange={() => { setRecipientMode('stores'); setSelectedProfileId(''); }} type="radio" /> 所选门店全体</label><label className={`rounded-lg border p-3 text-sm ${recipientMode === 'employee' ? 'border-brand-600 bg-brand-50' : ''}`}><input checked={recipientMode === 'employee'} onChange={() => setRecipientMode('employee')} type="radio" /> 单独指定员工</label></div>{recipientMode === 'employee' ? <label className="mt-3 block text-sm font-semibold">选择员工或店长<select className="ui-input mt-1" onChange={(event) => { const id = event.target.value; setSelectedProfileId(id); const recipient = recipients.find((item) => item.id === id); if (recipient) setStoreIds([recipient.store_id]); }} value={selectedProfileId}><option value="">请选择</option>{recipients.filter((recipient) => !selectedTemplate || selectedTemplate.storeIds.includes(recipient.store_id)).map((recipient) => <option key={recipient.id} value={recipient.id}>{recipient.display_name}（{recipient.role === 'manager' ? '店长' : '员工'}）· {auth.availableStores.find((store) => store.id === recipient.store_id)?.name}</option>)}</select></label> : null}</fieldset>
      ...
      {recipientMode === 'stores' ? <><div className="mt-3 flex flex-wrap gap-2">{auth.availableStores.map((store) => <label className="rounded-lg border p-3 text-sm" key={store.id}><input checked={storeIds.includes(store.id)} disabled={Boolean(selectedTemplate && !selectedTemplate.storeIds.includes(store.id))} onChange={() => setStoreIds(storeIds.includes(store.id) ? storeIds.filter((id) => id !== store.id) : [...storeIds, store.id])} type="checkbox" /> {store.name}</label>)}</div><p className="mt-2 text-xs text-slate-500">发布后将向所选门店内每位有效员工和店长发送任务通知。</p></> : <p className="mt-3 rounded-lg bg-amber-50 p-3 text-xs leading-5 text-amber-900">此任务仅对所选员工可见，其他同门店员工不会看到，也不会收到通知。</p>}
