...
export function AdminV2TasksPage() {
  ...
  return <PageShell eyebrow="门店运营系统 · 管理员" title="任务发布与审核" backTo="/app">
    <section className="grid grid-cols-2 gap-2"><Link className="flex min-h-11 items-center justify-center rounded-lg border bg-white font-bold text-brand-700" to="/app/admin/task-templates">任务模板</Link><button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-brand-600 font-bold text-white" onClick={() => setCreatorOpen((current) => !current)} type="button">创建任务<ChevronDown className={`h-4 w-4 transition ${creatorOpen ? 'rotate-180' : ''}`} /></button></section>
