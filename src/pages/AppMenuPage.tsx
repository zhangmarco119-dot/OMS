...
export function AppMenuPage() {
  ...
  return <PageShell eyebrow="门店运营系统" title="工作台" backTo="/app"><section className="grid grid-cols-3 gap-2 sm:gap-3">{items.map(({ icon: Icon, label, note, to }) => <Link className="flex aspect-square min-h-28 flex-col rounded-xl bg-slate-900 p-2.5 text-white shadow-sm ring-1 ring-slate-800 transition hover:bg-slate-800 active:scale-[0.98] sm:min-h-32 sm:p-3" key={to} to={to}><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/12 text-emerald-200 sm:h-10 sm:w-10"><Icon className="h-4 w-4 sm:h-5 sm:w-5" /></span><div className="mt-auto min-h-[3.25rem]"><b className="block text-sm leading-5 text-white sm:text-base">{label}</b><span className="mt-0.5 block min-h-8 text-[10px] leading-4 text-slate-300 sm:text-xs">{note}</span></div></Link>)}</section></PageShell>;
}
