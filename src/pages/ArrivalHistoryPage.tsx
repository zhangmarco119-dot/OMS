import { ChevronRight, PackageCheck, RefreshCw } from 'lucide-react';
...
export function ArrivalHistoryPage() {
  ...
  return (
    <PageShell eyebrow="门店运营系统" title="到货历史" backTo="/app/arrivals" contentGapClassName="gap-3">
      ...
      {status === 'ready' ? <div className="space-y-2.5">{items.map((report) => <Link aria-label={`查看到货记录 ${report.report_no}`} className="ui-card ui-interactive block p-4" key={report.id} to={`/app/arrivals/${report.id}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold text-brand-700">{report.report_no}</p><h2 className="mt-1 line-clamp-2 font-bold leading-6 text-slate-900">{report.generated_summary}</h2></div><div className="flex shrink-0 items-center gap-2"><StatusBadge tone={report.status === 'voided' ? 'danger' : report.status === 'draft' ? 'neutral' : 'success'}>{statusLabel[report.status]}</StatusBadge><ChevronRight className="h-4 w-4 text-slate-400" aria-hidden="true" /></div></div><p className="mt-2 text-sm text-slate-500">{report.reporter_name_snapshot} · 到货 {report.arrival_date} {report.arrival_time?.slice(0, 5) ?? ''}</p><p className="mt-1 text-xs text-slate-400">提交时间：{formatDateTime(report.submitted_at)}</p>{report.status === 'voided' ? <FeedbackBanner className="mt-3" title="作废原因" tone="danger">{report.void_reason}</FeedbackBanner> : null}</Link>)}</div> : null}
    </PageShell>
  );
}
