...
export function MyPayrollPage() {
  ...
  return <PageShell eyebrow="个人薪资" title="预估工资" backTo="/app/menu" contentGapClassName="gap-3">
    <label className="ui-card block p-3 text-sm font-semibold text-slate-700">查看月份<input className="ui-input mt-1" defaultValue={month} disabled={!settings} max={currentMonth()} min={historyEnabled && settings ? shiftMonth(currentMonth(), -settings.historyMonths) : currentMonth()} onChange={(event) => { if (/^\d{4}-\d{2}$/.test(event.target.value)) setMonth(event.target.value); }} step="1" type="month" /></label>
