...
export function OperationReportDetailPage() {
  ...
  return <PageShell backTo="/app/operation-reports" eyebrow={report.report_date} title={report.title_snapshot}><SectionCard><pre className="whitespace-pre-wrap text-sm leading-7 text-slate-700">{report.text_report}</pre><button className="ui-button-primary mt-4 w-full" onClick={async () => { await navigator.clipboard.writeText(report.text_report ?? ''); setCopied(true); }} type="button"><ClipboardCopy className="h-4 w-4" />一键复制纯文字</button></SectionCard>{canSeePhotos ? <SectionCard><SectionHeader icon={Images} title="现场图片" description="店长和管理员可查看报告所附的物料与报废照片。" /><div className="mt-3 grid grid-cols-2 gap-2">{images.map((image) => { const field = report.field_config_snapshot.find((item) => item.id === image.field_id); const value = report.manual_values[image.field_id]?.trim(); return <figure key={image.id}><img alt="运营报告现场图片" className="aspect-square w-full rounded-xl bg-slate-50 object-contain" src={image.signedUrl} /><figcaption className="mt-1 text-center text-xs text-slate-600">{field?.label ?? image.field_id}{value ? `：${value}${field?.unit ?? ''}` : ''}</figcaption></figure>; })}</div></SectionCard> : null}<ActionFeedbackDialog message="纯文字运营报告已复制到剪贴板。" onClose={() => setCopied(false)} open={copied} title="复制成功" tone="success" /></PageShell>;
}
