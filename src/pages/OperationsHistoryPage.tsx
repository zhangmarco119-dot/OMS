import { ClipboardList, History, PackageCheck, RefreshCw } from 'lucide-react';
...
export function OperationsHistoryPage() {
  const auth = useAuth();
  const [counts, setCounts] = useState({ arrivals: 0, legacy: 0 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [legacy, arrivals] = await Promise.all([
        loadSubmittedTasks(supabase, auth.profile),
        arrivalCount,
      ]);
      if (arrivals.error) throw new Error(arrivals.error.message);
      setCounts({ arrivals: arrivals.count ?? 0, legacy: legacy.length }); setMessage(null);
