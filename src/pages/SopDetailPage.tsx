...
export function SopDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const found = loaded?.status === 'published' ? loaded : null;
      setSop(found);
      setStatus('ready');
      setMessage(null);
    } catch (error) {
  ...
  return <PageShell eyebrow={sop?.category ?? 'SOP 手册'} title={sop?.title ?? 'SOP 详情'} backTo="/app/sops" contentGapClassName="gap-3">
    ...
    {sop ? <article className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      {images.length ? <div>{images.map((asset, index) => <section className="border-b border-slate-100 last:border-b-0" key={asset.id}><div className="bg-slate-50 px-4 py-2 text-sm font-bold text-brand-700">步骤 {index + 1}</div><button aria-label={`放大查看步骤 ${index + 1} 图片`} className="block w-full bg-white" onClick={() => setActiveImage({ alt: asset.file_name, url: asset.signedUrl })} type="button"><img alt={`${sop.title} 步骤 ${index + 1}`} className="h-auto w-full object-contain" src={asset.signedUrl} /></button><p className="whitespace-pre-wrap px-4 py-4 text-[15px] leading-7 text-slate-800">{asset.step_text || `请按图示完成第 ${index + 1} 步。`}</p></section>)}</div> : <section className="p-5"><p className="text-sm text-slate-500">该 SOP 暂无分步图片，请按整体说明执行。</p></section>}
