...
import { loadManagerOvertimeRequests } from '../services/payroll.service';
...
export function TodoPage() {
  ...
  const load = useCallback(async () => {
    if (!supabase) return;
    try {
      const [nextTasks, summary, nextNotices, nextFeedback, nextOvertime] = await Promise.all([
  ...
  return <PageShell eyebrow="门店运营系统" title="待办" contentGapClassName="gap-3">
    ...
    {overtime.length > 0 ? <section className="space-y-2"><h2 className="text-sm font-bold text-slate-700">加班审批</h2>{overtime.map((item) => <Link className="ui-card ui-interactive block p-4" key={item.id} to="/app/overtime?view=approvals"><div className="flex items-start justify-between gap-3"><b>{item.overtime_date} · {item.hours} 小时加班申请</b><StatusBadge tone="warning">待审批</StatusBadge></div><p className="mt-2 text-sm text-slate-500">{item.reason}</p></Link>)}</section> : null}
    ...
    {tasks.length === 0 && feedbackCount === 0 && notices.length === 0 && overtime.length === 0 ? <EmptyState description="新的任务审核、货品申请、加班审批或需确认公告会显示在这里。" icon={CheckCircle2} title="当前没有待办" /> : null}
  </PageShell>;
}
