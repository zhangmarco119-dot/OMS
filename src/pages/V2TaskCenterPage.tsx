...
export function V2TaskCenterPage() {
  ...
  const load = useCallback(async () => {
    ...
    setStatus('loading');
    try { const [nextTemplates,nextTasks]=await Promise.all([loadPublishedTemplatesForStore(supabase, auth.store.id),loadV2Tasks(supabase,auth.store.id)]);setTemplates(nextTemplates);setTasks(nextTasks); setStatus('ready'); setMessage(null); }
  ...
  return <PageShell eyebrow="StoreHub V2 · 阶段 5" title="任务中心" backTo="/app">
    <section className="rounded-lg border border-brand-100 bg-brand-50 p-4"><div className="flex gap-3"><ShieldCheck className="h-6 w-6 shrink-0 text-brand-700" /><div><h2 className="font-bold text-brand-900">门店任务</h2><p className="mt-1 text-sm leading-6 text-brand-800">任务自动保存，可提交审核；被退回后按指定项目整改并重新提交。</p></div></div></section>
    ...
    {status === 'ready' && tasks.length === 0 ? <div className="rounded-lg bg-white p-8 text-center shadow-sm"><ClipboardCheck className="mx-auto h-12 w-12 text-slate-300" /><p className="mt-4 font-bold text-slate-900">当前门店暂无任务</p><p className="mt-2 text-sm text-slate-500">管理员发布任务后会显示在这里。</p></div> : null}
    {status==='ready'?<div className="space-y-3">{tasks.map(task=><Link className="block rounded-lg bg-white p-4 shadow-sm" key={task.id} to={`/app/tasks/${task.id}`}><div className="flex justify-between gap-3"><b>{task.name}</b><span className="text-sm text-brand-700">{task.status}</span></div><p className="mt-2 text-sm text-slate-500">截止 {new Date(task.due_at).toLocaleString('zh-CN')}</p>{task.status==='rejected'?<p className="mt-2 text-sm text-red-700">已退回：{task.review_note}</p>:null}</Link>)}</div>:null}
