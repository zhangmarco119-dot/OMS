...
function AnswerCard({ answer, deletingImageIds, editable, images, imageUrls, imageUrlsLoading, needsCorrection, number, onChange, onDeleteImage, onUpload, referenceImageUrls, referenceImageUrlsLoading, uploaderId }: { answer: V2TaskAnswerRow; deletingImageIds: string[]; editable: boolean; images: V2TaskImageRow[]; imageUrls: Record<string, string>; imageUrlsLoading: boolean; needsCorrection: boolean; number: string; onChange: (value: Json) => void; onDeleteImage: (image: V2TaskImageRow) => void; onUpload: (file: File | undefined) => void; referenceImageUrls: string[]; referenceImageUrlsLoading: boolean; uploaderId: string }) {
  ...
  const productValue = ['product_spec', 'product_correction'].includes(item.answer_schema ?? '') && value && !Array.isArray(value) && typeof value === 'object'
    ? value as Record<string, Json | undefined>
    : {};
  const updateProductValue = (updates: Record<string, Json>) => onChange({
    category_code: typeof productValue.category_code === 'string' ? productValue.category_code : '',
    count_unit: typeof productValue.count_unit === 'string' ? productValue.count_unit : '',
    name: typeof productValue.name === 'string' ? productValue.name : '',
  ...
  return <article className={`ui-card p-4 ${needsCorrection ? 'border-red-300 bg-red-50/20' : !editable && answer.review_status === 'approved' ? 'border-emerald-200 bg-emerald-50/20' : ''}`}><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-bold text-slate-600">{number}</span><h2 className="font-bold">{item.label}{item.is_required ? <span className="ml-1 text-red-600">*</span> : ''}</h2>{needsCorrection ? <span className="rounded-full border border-red-200 bg-red-50 px-2 py-1 text-xs font-bold text-red-700">需整改</span> : !editable && answer.review_status === 'approved' ? <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-bold text-emerald-800">已通过 · 无需修改</span> : null}</div>{item.guidance ? <p className="mt-1 text-sm leading-5 text-slate-500">{item.guidance}</p> : null}{needsCorrection && answer.note ? <p className="mt-2 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-800">驳回原因：{answer.note}</p> : null}<TaskReferenceImagePreview loading={referenceImageUrlsLoading} urls={referenceImageUrls} /><div className="mt-3">
    {item.field_type === 'instruction' ? <p className="rounded bg-slate-50 p-3 text-sm">请按说明完成本项。</p> : null}
    {item.answer_schema === 'product_spec' ? <div className="space-y-3"><label className="block text-sm font-semibold text-slate-700">正确规格<input aria-label={`${item.label}正确规格`} className="ui-input mt-1" disabled={!editable} onChange={(event) => onChange({ count_unit: typeof productValue.count_unit === 'string' ? productValue.count_unit : '', spec: event.target.value })} placeholder={item.current_spec ? `当前：${item.current_spec}` : '例如：500g/袋×20袋/箱'} value={typeof productValue.spec === 'string' ? productValue.spec : ''} /></label><label className="block text-sm font-semibold text-slate-700">点货单位<input aria-label={`${item.label}点货单位`} className="ui-input mt-1" disabled={!editable} onChange={(event) => onChange({ count_unit: event.target.value, spec: typeof productValue.spec === 'string' ? productValue.spec : '' })} placeholder={item.current_count_unit ? `当前：${item.current_count_unit}` : '例如：袋、盒、罐'} value={typeof productValue.count_unit === 'string' ? productValue.count_unit : ''} /></label></div> : null}
    {item.answer_schema === 'product_correction' ? <div className="space-y-3"><label className="block text-sm font-semibold text-slate-700">正确名称<input aria-label={`${item.label}正确名称`} className="ui-input mt-1" disabled={!editable} onChange={(event) => updateProductValue({ name: event.target.value })} value={typeof productValue.name === 'string' ? productValue.name : ''} /></label><label className="block text-sm font-semibold text-slate-700">正确规格<input aria-label={`${item.label}正确规格`} className="ui-input mt-1" disabled={!editable} onChange={(event) => updateProductValue({ spec: event.target.value })} placeholder="例如：500g/袋×20袋/箱" value={typeof productValue.spec === 'string' ? productValue.spec : ''} /></label><label className="block text-sm font-semibold text-slate-700">点货单位<input aria-label={`${item.label}点货单位`} className="ui-input mt-1" disabled={!editable} onChange={(event) => updateProductValue({ count_unit: event.target.value })} placeholder="例如：袋、盒、罐" value={typeof productValue.count_unit === 'string' ? productValue.count_unit : ''} /></label><label className="block text-sm font-semibold text-slate-700">货品分类<select aria-label={`${item.label}货品分类`} className="ui-input mt-1" disabled={!editable} onChange={(event) => updateProductValue({ category_code: event.target.value })} value={typeof productValue.category_code === 'string' ? productValue.category_code : ''}><option value="">请选择分类</option>{PRODUCT_CATEGORIES.map((category) => <option key={category.code} value={category.code}>{category.label}</option>)}</select></label></div> : null}
    {!['product_spec', 'product_correction'].includes(item.answer_schema ?? '') && ['short_text', 'long_text'].includes(item.field_type) ? <textarea className="ui-input min-h-20 py-3" disabled={!editable} onChange={(event) => onChange(event.target.value)} value={typeof value === 'string' ? value : ''} /> : null}
