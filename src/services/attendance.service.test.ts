...
describe('attendance service', () => {
  const clientWithEmptyOvertime = (rpc: ReturnType<typeof vi.fn>) => {
    const query = {
      ...
      gte: vi.fn(),
      lt: vi.fn(),
      then: (resolve: (value: { data: never[]; error: null }) => unknown) => Promise.resolve({ data: [], error: null }).then(resolve),
    };
  ...
  it('parses employee month details from the stable RPC contract', async () => {
    ...
    const result = await loadAttendanceMonth(clientWithEmptyOvertime(rpc) as never, 'profile-1', '2026-07');
  ...
  it('passes administrator pagination and filters only to the protected RPC', async () => {
    ...
    const result = await loadAdminAttendanceMonth(clientWithEmptyOvertime(rpc) as never, { month: '2026-07', storeId: 's1', search: '员工', status: 'normal', offset: 50 });
