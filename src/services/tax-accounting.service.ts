...
import { allocatePayrollCosts, type StoreCostAllocation } from '../features/tax-accounting/allocation';
...
export async function loadTaxAccountingData(client: Client, month: string): Promise<TaxAccountingData> {
  ...
  const firstError = storesResult.error || peopleResult.error || salariesResult.error || attendanceResult.error || overtimeResult.error;
  ...
  const allocations = allocatePayrollCosts(
  ...
  return {
    ...
    profiles,
    stores,
    taxReports: createTaxReports(stores, people, monthlySalaries, payslips),
  };
}
...
export async function saveTaxPerson(client: Client, actorId: string, input: SaveTaxPersonInput) {
  ...
  const query = input.id
    ? client.from('tax_reporting_people').update(values).eq('id', input.id)
    : client.from('tax_reporting_people').insert({ ...values, created_by: actorId });
