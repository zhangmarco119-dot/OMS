...
export const loadTodoSummary = async (client: Client, input: { isAdmin: boolean; isManager?: boolean; profileId: string; storeId?: string; storeIds?: string[] }): Promise<TodoSummary> => {
  if (input.isAdmin) {
    ...
    return { count: taskCount + feedbackCount + overtimeCount, noticeAcknowledgements: 0, productFeedback: feedbackCount, tasks: taskCount, overtime: overtimeCount, attendanceCorrections: 0 };
  }
  const [tasks, acknowledgements, overtime, corrections] = await Promise.all([
  ...
  return { count: taskCount + acknowledgementCount + overtimeCount + correctionCount, noticeAcknowledgements: acknowledgementCount, productFeedback: 0, tasks: taskCount, overtime: overtimeCount, attendanceCorrections: correctionCount };
};
