...
begin
 ...
 select count(*) into v_missing from public.v2_task_answers a
 where a.task_id=p_task_id and coalesce((a.item_snapshot->>'is_required')::boolean,true)
 and case coalesce(a.item_snapshot->>'field_type','')
   when 'instruction' then false
   when 'image' then not exists(select 1 from public.v2_task_images i where i.task_id=a.task_id and i.item_id=a.item_id)
   when 'multi_image' then not exists(select 1 from public.v2_task_images i where i.task_id=a.task_id and i.item_id=a.item_id)
 ...
 values('admin',v_task.store_id,'v2_task_submitted','任务待审核',v_task.name,'v2_task',v_task.id,'v2-task-submitted:'||v_task.id||':'||v_task.version) on conflict(dedupe_key) do nothing;
