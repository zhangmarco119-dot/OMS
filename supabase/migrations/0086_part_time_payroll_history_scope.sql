-- Keep historical payroll results scoped to employees who already existed or
-- who have attendance/overtime data in the selected month. Part-time employees
-- must not appear in every month before their account was created.
...
begin
  ...
  with targets as (
    ...
    where profile.role in ('staff', 'manager')
      ...
      and (
        ...
        or exists (
          ...
          where daily.profile_id = profile.id
            and daily.attendance_date between v_month_start and v_month_end
        )
        or exists (
          select 1
          from public.payroll_overtime_requests overtime
          where overtime.profile_id = profile.id
            and overtime.overtime_date between v_month_start and v_month_end
        )
      )
      and (
        p_store_id is null
    ...
    select public.get_payroll_estimate(target.id, p_as_of) estimate
    from targets target
  )
