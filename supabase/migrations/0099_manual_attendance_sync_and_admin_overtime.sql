-- Manual attendance synchronization no longer reuses a recent completed job.
-- The Edge Function always applies the incremental day planner. This migration
-- adds the permission-protected administrator workflow for directly recording
-- approved employee overtime.
...
begin
  if public.current_user_role() <> 'admin' then
    raise exception '仅管理员可以手动登记员工加班工时';
  end if;
  ...
  if v_profile.role not in ('staff', 'manager') or v_profile.employment_type <> 'full_time' then
    raise exception '只能为全职员工或店长登记加班工时';
  end if;
  if not (
  ...
  if p_overtime_date is null or p_overtime_date > v_today then
    raise exception '加班日期不能晚于今天';
  end if;
  if p_hours is null or p_hours <= 0 or p_hours > 6 or mod(p_hours, 0.5) <> 0 then
    raise exception '加班工时必须按 0.5 小时递增，且在 0.5 至 6 小时之间';
  end if;
  ...
  limit 1;
  if v_rate is null then raise exception '尚未配置适用于该日期的加班时薪'; end if;
