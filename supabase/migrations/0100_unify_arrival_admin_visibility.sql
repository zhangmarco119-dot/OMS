-- Keep the dashboard arrival total aligned with the default Arrival Center list.
-- Only formally submitted, non-voided reports are effective arrivals.
...
begin
  ...
  return query select
    (
      ...
      where report.arrival_date = local_today
    ...
    (
      ...
      where task.task_type = 'inventory'
        and task.status = 'submitted'
        and task.submitted_at >= (local_today::timestamp at time zone 'Asia/Shanghai')
    ...
    (
      ...
      from public.tasks task
      where task.task_type = 'inventory'
        and task.status in ('draft', 'review')
        and public.has_store_access(task.store_id)
    ),
    (
      select count(*)::integer
    ...
    (
      select count(*)::integer
      from public.v2_tasks task
      where task.status in ('pending', 'in_progress', 'submitted', 'rejected', 'resubmitted', 'overdue')
        and public.has_store_access(task.store_id)
    );
end;
$$;
