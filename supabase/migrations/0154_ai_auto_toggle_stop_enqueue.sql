-- When automatic analysis is off, stop enqueueing every trigger type.
-- ensure/rerun are still entered from the admin UI, but they must not start
-- any model work while the global switch is off.
...
begin
  ...
  if not v_settings.auto_run_enabled then
    return jsonb_build_object('status', 'disabled', 'deduplicated', false);
  end if;
  if (
