-- Let administrators search the paginated Arrival Center by a product name
-- without limiting matches to the currently visible page.
create or replace function public.list_admin_arrivals_v2(
...
declare
  ...
  v_product_search text := lower(trim(coalesce(p_product_search, '')));
  ...
  with scoped as materialized (
    ...
    where public.has_store_access(report.store_id)
      ...
      and (
        ...
        or exists (
          select 1
          from public.arrival_report_items searched_item
          where searched_item.report_id = report.id
            and position(v_product_search in lower(searched_item.product_name_snapshot)) > 0
        )
      )
  ), paged as (
    select report.*
...
revoke all on function public.list_admin_arrivals_v2(date,date,uuid,text,text,integer,integer) from public,anon;
grant execute on function public.list_admin_arrivals_v2(date,date,uuid,text,text,integer,integer) to authenticated;

comment on function public.list_admin_arrivals_v2(date,date,uuid,text,text,integer,integer) is
  'Returns one authorized, paginated Arrival Center payload filtered by date, store, status, and optional product name.';
